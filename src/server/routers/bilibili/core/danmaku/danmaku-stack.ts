...
export class DanmakuStack {
  ...
  getTextSize(danmaku: Danmaku) {
    // 改进的估算方法：区分中英文（全角/半角）字符
    let totalWidth = 0;
    for (const char of danmaku.content) {
      // 使用正则表达式判断是否为全角字符（包括中日韩文字、全角符号等）
      if (/[\u4e00-\u9fa5\uFF00-\uFFFF]/.test(char)) {
        totalWidth += 1; // 全角字符宽度计为1
      } else {
        totalWidth += 0.5; // 半角字符（英文、数字、半角符号）宽度计为0.5
      }
    }

    const estimatedWidth = totalWidth * danmaku.fontSize; // 乘以字体大小得到估算像素宽度
    const x = estimatedWidth / 2;
  ...
  getHorizontalTags(danmaku: Danmaku) {
    return this.getTags(danmaku, {
      ...
      getTag: ({ trackNumber, x, y }) =>
        `\\move(${this.resolution.x + x},${trackNumber * this.trackHeight + DanmakuStack.margin + y},${-x},${trackNumber * this.trackHeight + DanmakuStack.margin + y},0,${this.duration(danmaku) * 1000})`
    });
  }
